...
class OntologyGenerator:
    ...
    # Max chars fed to LLM — keeps total prompt within model context window
    MAX_TEXT_LENGTH_FOR_LLM = 6000   # ~1500 tokens of document text
    MAX_SIM_REQ_LENGTH = 800         # simulation requirement summary only
    ...
    def _build_user_message(
        ...
        # Truncate document text to fit model context window
        if len(combined_text) > self.MAX_TEXT_LENGTH_FOR_LLM:
            combined_text = combined_text[:self.MAX_TEXT_LENGTH_FOR_LLM]
            combined_text += f"\n\n...[truncated: {original_length} chars total]"

        # Truncate simulation_requirement if it's a long document rather than a short question
        if len(simulation_requirement) > self.MAX_SIM_REQ_LENGTH:
            simulation_requirement = simulation_requirement[:self.MAX_SIM_REQ_LENGTH] + "..."
...
"""
    ...
    def generate_python_code(self, ontology: Dict[str, Any]) -> str:
        ...
        Not used in MegaFish (ontology stored as JSON in Neo4j).
        Kept for reference only.
        """
        code_lines = [
            '"""',
            'Custom entity type definitions',
            'Auto-generated by MegaFish for social opinion simulation',
            '"""',
            '',
